...
public class LoginUserServlet extends LoginBaseServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (request.getParameter("logout") != null) {
			...
			String lastTempTime = dbhandler.getLastTempTime(user);
			dbhandler.updateTimeToDisplay(user, lastTempTime);
		...
		try {
			if (status == Status.OK) {
				...
				if(!getCookieMap(request).containsKey("loginTime")) {
					response.addCookie(new Cookie("loginTime", "true"));
					dbhandler.updateTimeTemp(user, getDate());
				}
